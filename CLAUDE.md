...
[A] UI 텍스트 작성 시 사전 자가 검토
   ...
   4-1. (2026-07-07 사용자 명령) dry-run / 드라이런 / dedup / 백필 / 파이프라인 등
        개발 프로세스 용어도 화면·확인창·사용자 대화에서 금지 — 쉬운 말로:
        「미리 확인」 「중복 정리」 「예전 자료 채우기」 「자동 처리 흐름」.
        확인창은 "무엇을 N건 어떻게 할까요? (복구 가능 여부)" 한 문장 패턴.
...
### 규칙 30 — 테이블 컬럼 묶어쓰기 금지 (2026-07-07 신설)

> **사용자 명령**: 「컬럼 좀 묶어쓰지 말자. 깔끔하게 하나씩 분리하는 스타일로.
>   자꾸 두세 개 붙여서 묶어쓰는 스타일이 나와서 맘에 안 드네」

**원칙**: 테이블 컬럼은 **1컬럼 = 1값**. 규칙 19(자리 절약)보다 우선.

```
✗ 헤더에 「입금액 · 입금일」 「과실·청구율」 「사고차량 · 대차차량 · 고객」 — 금지
✗ 한 셀에 값 2~3개 (금액+날짜, 유형+퍼센트+금액 등) — 금지
✓ 각각 별도 컬럼으로 분리 (좁은 width 로 자리 확보)
✓ 자리가 부족하면 부차 정보는 컬럼 삭제 → 드로어/카드/상세에서 표시
✓ 예외: 상태 배지 1개 + 뒤에 붙는 보조 아이콘 1개까지, 액션 버튼 묶음 컬럼
```

**확장 (2026-07-08 사용자 명령)**: 「2중 3중 데이터를 표출하지 마라. 전부 컬럼 단위로.
PC 웹은 데이터 오류·오인이 없게 각각 관리. 모바일은 나중에 필요 부분만 선택.」

```
✗ 한 셀에 여러 줄 쌓기 (은행명 / 별칭 / 상태라벨 3층 등) — 금지
✗ 같은 정보를 다른 컬럼·라벨로 반복 표출 — 금지
✓ PC 웹 = 1셀 1값 컬럼 분리가 기준. 모바일 대응은 mobileCard 로 별도 구성 (컬럼 훼손 X)
✓ 등록/미등록 같은 관리 상태는 관리 탭(매핑 관리)에서 — 원장 리스트에 경고 라벨 반복 X
```
