    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '0 20px', borderBottom: '1px solid #e2e8f0', background: '#fff' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 16 }}>
          {[
            ...
            { label: '월 지급', value: fm(monthlyTotal), color: '#2563eb' },
          ].map(s => (
          ...
          {activeSubTab !== 'all' && (
            <button onClick={() => router.push(activeSubTab === 'jiip' ? '/jiip/new' : '/invest/general/new')}
              style={{ background: '#2563eb', color: '#fff', border: 'none', padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', marginLeft: 4 }}>
              + 신규 등록
            </button>
          )}
        </div>
            ...
            return (
              <div key={groupKey}>
                ...
                <div onClick={() => toggleGroup(groupKey)}
                  ...
                  <span style={{ fontSize: 12, color: '#2563eb', fontWeight: 700 }}>{fm(group.totalAmount)}원</span>
                  <span style={{ fontSize: 11, color: '#94a3b8' }}>·</span>
                  ...
                  if (kind === 'jiip') {
                    ...
                    return (
                      <div key={item.id} style={rowBase}
                        ...
                        <div style={{ width: 120, flexShrink: 0, textAlign: 'right' }}>
                          <div style={{ fontWeight: 800, fontSize: 13, color: '#111827' }}>{f(item.invest_amount)}</div>
                          <span style={{ fontSize: 10, background: '#eff6ff', color: '#2563eb', padding: '1px 5px', borderRadius: 3, fontWeight: 700 }}>배분 {item.share_ratio}%</span>
                        </div>
                        {/* 월 관리비 */}
                        <div style={{ width: 110, flexShrink: 0, textAlign: 'right' }}>
                    ...
                    return (
                      <div key={item.id} style={rowBase}
                        ...
                        <div style={{ width: 130, flexShrink: 0, textAlign: 'right' }}>
                          <div style={{ fontWeight: 800, fontSize: 13, color: '#111827' }}>{f(item.invest_amount)}</div>
                          <span style={{ fontSize: 10, background: '#eff6ff', color: '#2563eb', padding: '1px 5px', borderRadius: 3, fontWeight: 700 }}>연 {N(item.interest_rate).toFixed(1)}%</span>
                        </div>
                        {/* 월 이자(세후) */}
                        <div style={{ width: 120, flexShrink: 0, textAlign: 'right' }}>
      ...
      {filteredList.length > 0 && (
        <div style={{ padding: '10px 20px', borderTop: '1px solid #e5e7eb', background: '#f8fafc', display: 'flex', gap: 16, fontSize: 11, color: '#64748b', fontWeight: 600, flexWrap: 'wrap' }}>
          ...
          <span>누적 지급 <b style={{ color: '#2563eb' }}>{fm(summaryStats.totalPaid)}</b></span>
