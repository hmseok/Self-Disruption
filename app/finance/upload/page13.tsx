...
export default function UploadFinancePage() {
  ...
  const processFiles = async (files: FileList | File[]) => {
    ...
    for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const progressMsg = `[${i + 1}/${files.length}] '${file.name}' 분석 중...`;
        setLoadingMsg(progressMsg);

        try {
            await processSingleFile(file); // 개별 파일 처리 함수 호출
            successCount++;
        } catch (e: any) {
    ...
    setLoading(false);
    alert(`✅ 총 ${files.length}개 파일 중 ${successCount}개 처리 완료!`);
  }

  // 📄 개별 파일 처리 로직 (이전 processFile의 내용을 분리)
  const processSingleFile = async (file: File) => {
    ...
    if (file.name.match(/\.(xlsx|xls|csv)$/i) || file.type.includes('spreadsheet') || file.type.includes('csv')) {
        ...
        if (jsonData.length < 2) return; // 데이터 없음
  ...
  // 👋 [수정] 여러 파일을 받도록 핸들러 변경
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files.length > 0) {
          processFiles(Array.from(e.target.files)); // FileList를 배열로 변환해서 전달
      }
      e.target.value = '';
  }
  ...
  const onDrop = (e: React.DragEvent) => {
      ...
      if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
          processFiles(Array.from(e.dataTransfer.files)); // 여러 파일 전달
      }
  }
