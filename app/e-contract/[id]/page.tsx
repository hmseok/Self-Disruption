    ...
    <div style={{ padding: '24px 16px', minHeight: '100vh', background: '#f9fafb' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {contract.status === 'draft' && (
              <>
                ...
                <button onClick={() => handleStatusChange('pending_sign')} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#2563eb', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>📤 발송하기</button>
              </>
            )}
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 16 }}>
        {(['info', 'preview'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '8px 20px', borderRadius: '8px 8px 0 0', border: 'none', background: tab === t ? '#fff' : '#e5e7eb', fontWeight: tab === t ? 700 : 500, fontSize: 13, cursor: 'pointer', color: tab === t ? '#111827' : '#6b7280', borderBottom: tab === t ? '2px solid #2563eb' : '2px solid transparent' }}>
      ...
      {tab === 'info' ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e5e7eb' }}>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: '#111827', marginBottom: 12, borderLeft: '4px solid #2563eb', paddingLeft: 10 }}>임차인 정보</h3>
            {[
