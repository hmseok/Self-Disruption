...
export default function EContractGuestSignPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/e-contract/guest-sign?contract_id=${id}`)
        if (!res.ok) {
  ...
  const handleSaveSignature = async () => {
    ...
    try {
        ...
        // 3. HTML -> 이미지 -> PDF 변환 (2페이지)
        const page1 = hiddenContractRef.current.querySelector('#rental-contract-page1') as HTMLElement
        const page2 = hiddenContractRef.current.querySelector('#rental-contract-page2') as HTMLElement

        if (!page1 || !page2) throw new Error("계약서 페이지를 찾을 수 없습니다")

        const pdf = new jsPDF('p', 'mm', 'a4')
        const pdfWidth = 210

        // 페이지 1
        const img1 = await toPng(page1, { cacheBust: true, backgroundColor: '#ffffff' })
        const props1 = pdf.getImageProperties(img1)
        const height1 = (props1.height * pdfWidth) / props1.width
        pdf.addImage(img1, 'PNG', 0, 0, pdfWidth, height1)

        // 페이지 2
        pdf.addPage('a4')
        const img2 = await toPng(page2, { cacheBust: true, backgroundColor: '#ffffff' })
        const props2 = pdf.getImageProperties(img2)
        const height2 = (props2.height * pdfWidth) / props2.width
        ...
        formData.append('file', pdfBlob, `e-contract_${id}_${Date.now()}.pdf`)

        const res = await fetch('/api/e-contract/guest-sign', {
          method: 'POST',
          body: formData,
        })
  ...
  // 이미 서명된 경우
  if (contract?.signed_at && !completed) {
    return (
      <div className="fixed inset-0 z-[99999] bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
          <div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-sm border border-gray-100">
    ...
    <div className="fixed inset-0 z-[99999] bg-gray-100 flex flex-col overflow-hidden">
      ...
      <div style={{ position: 'absolute', top: '-10000px', left: '-10000px' }}>
          <div ref={hiddenContractRef}>
              {contract && (
                <>
                  <div id="rental-contract-page1">
                    <RentalContractPaper data={{ ...contract, renter_signature_url: tempSignature }} forPdf />
                  </div>
                  <div id="rental-contract-page2">
                    <RentalContractTerms companyName={contract.company_name} renterName={contract.renter_name} signatureUrl={tempSignature} forPdf />
                  </div>
                </>
              )}
          </div>
      ...
      <div className="bg-slate-900 text-white p-4 text-center flex-none shadow-md z-10">
          <h1 className="font-bold text-lg">임대차 계약 서명</h1>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-500 p-4 pb-10">
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden min-h-[300px] flex justify-center items-start pt-4 mb-4">
              <div className="origin-top transform scale-[0.45] md:scale-100">
                {contract && <RentalContractPaper data={contract} />}
              </div>
          </div>
          ...
          <section className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
              <h3 className="font-bold text-gray-900 text-lg mb-4">임대차 정보</h3>
              <div className="space-y-3 text-sm">
                  <div className="flex justify-between py-2 border-b border-gray-50">
                      <span className="text-gray-500">차량 번호</span>
                      <span className="font-bold text-gray-900">{contract?.car_number || '-'}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-gray-50">
                      <span className="text-gray-500">차종/모델</span>
                      <span className="font-bold text-gray-900">{contract?.car_model || '-'}</span>
                  </div>
                  {contract?.dispatch_at && (
                    <div className="flex justify-between py-2 border-b border-gray-50">
                        <span className="text-gray-500">대여 기간</span>
                        <span className="font-bold text-gray-900">{contract.dispatch_at.slice(0, 10)} ~ {contract.return_at?.slice(0, 10) || '-'}</span>
                    </div>
                  )}
              </div>
          </section>
      ...
      {showZoomModal && (
        <div className="fixed inset-0 z-[100000] bg-black/90 flex flex-col animate-fade-in">
            ...
            <div className="flex-1 overflow-auto p-4 bg-gray-900 flex justify-center">
                <div className="space-y-8 bg-white shadow-2xl min-w-[210mm] p-0">
                    {contract && (
                      <>
                        ...
                        <div className="min-h-[297mm]">
                          <RentalContractTerms companyName={contract.company_name} renterName={contract.renter_name} />
                        </div>
                      </>
                    )}
                </div>
