...
import {
  Button, Field, FilterPill, KpiCard, KpiRow, PageHeader, ScreenWrap,
  Select, Spinner, StatusBadge, TextInput,
} from '../_components/ui'
...
export default function IntakeMain() {
  ...
  const search = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/RideEmployees/factory-map/api/factories?${params}`).then(r => r.json())
      ...
      Promise.all(ranked.map(async f => {
        try {
          const url = `/RideEmployees/factory-map/api/directions?origin=${o.lng},${o.lat}&destination=${f.lng},${f.lat}`
  ...
  return (
    <ScreenWrap>
      ...
      <div className="px-6 pb-8 grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-4">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl ring-1 ring-slate-200 p-10 text-center">
              ...
              <div className="text-[12px] text-slate-500 mt-2 leading-6">
                다음 중 하나를 시도해보세요:<br />
                · 보험/차량 조건을 완화합니다<br />
                · <b className="text-amber-700">즐겨찾기 공장 좌표 등록 필요</b>
              </div>
            </div>
          ) : (
        ...
        <aside className="bg-white rounded-2xl ring-1 ring-slate-200 overflow-hidden h-[500px] sticky top-4 relative">
