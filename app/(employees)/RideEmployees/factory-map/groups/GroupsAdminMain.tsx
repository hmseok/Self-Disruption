...
// ───────────────────────────────────────────────────────────────
// 분류 셋팅 — 6축 종합 관리 (격리본)
// localStorage('ride_op_classifications_v2') 에 저장 — 페이지 단위 격리
// ───────────────────────────────────────────────────────────────
...
export default function GroupsAdminMain() {
  ...
  const addItem = (axisKey: string) => {
    // 이벤트 핸들러 내부에서만 호출되므로 render 중 호출 아님 — lint 회피
          ...
          <ul className="text-[12px] text-slate-600 space-y-1 list-disc list-inside leading-6">
            <li><b>편집 가능 범위</b>는 축마다 다름:
              <ul className="list-disc list-inside ml-4 mt-1">
                <li><b>전체 편집</b>: 즐겨찾기 그룹, 차량 분류, 정산 구분 등</li>
                <li><b>라벨만 편집</b>: 보험 입고, 공장 유형, 특수 태그 (키는 데이터 매핑 때문에 고정)</li>
              </ul>
            </li>
            <li><b>표시</b> 끄면 카카오 지도/필터/리스트에서 해당 코드 값이 기본 숨겨짐.</li>
            <li><b>색상</b>은 마커/뱃지 표시색에 반영.</li>
            <li><b>저장</b>은 브라우저 localStorage 에만 보관 — 서버 동기화는 메인 ERP 연동 시 별도 코드 마스터에 반영.</li>
          </ul>
        </Section>
      </div>
    </ScreenWrap>
