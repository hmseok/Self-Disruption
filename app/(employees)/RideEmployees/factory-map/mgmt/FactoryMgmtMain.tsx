...
export default function FactoryMgmtMain() {
  ...
  const load = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      const res = await fetch(`/RideEmployees/factory-map/api/factories?${p}`)
  ...
  return (
    <ScreenWrap>
      <PageHeader
        ...
        emoji="🔧"
        right={
          <Link href="/RideEmployees/factory-map" className="px-4 py-2 bg-blue-600 text-white text-[13px] font-semibold rounded-lg hover:bg-blue-700">
            🗺️ 지도에서 보기
          </Link>
        }
      />
