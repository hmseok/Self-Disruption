...
> 사용자 명령:
>   ① 「버전,최소인원,우선순위 정책 음 없어도 될것같고」
>   ② 「각 카드를 전체로 놓을 필요없을것같은데 미리보기를 제일 밑에 두고 접었다 펴고」
>   ③ 「멤버는 위쪽으로 올리고 기본 구성하고 같이 구분하여두고」
>   ④ 「쓸데없이 설명에 긴내용들도 정리해서 간단한 선택이나 라이도 스위치 토글가능한것은 토글써서 많이 줄여보자 심플하게」.

- **3 박스 제거** (UI 노출 OFF — DB/state 보존 for backward compat):
  - 📅 버전 timeline → `false && !isNew && ...` guard
  - ⚖️ 최소 인원 collapsible → `false && (<>...</>)` guard
  - 🎯 우선순위 정책 → `false && (<>...</>)` guard
  - 향후 admin 전용 화면이나 별도 탭에서 노출 가능 (요청 시).
- **2-column grid 레이아웃**:
  - 외곽 wrapper 를 `flex column` → `grid: minmax(360px,1fr) minmax(360px,1fr)` 로 변경.
  - 좌 = 기본 정의 카드 (이름/카테고리/시프트/패턴/전략/색상·설명/공휴일/협업/로테이션 토글).
  - 우 = 멤버 카드 (선택된 멤버 + 후보).
  - 「멤버는 위쪽으로 올리고」 — 멤버 카드가 grid 우측에 나란히 위치 (이전: 하단 분리).
- **매트릭스 → 하단 collapsible**:
  - `matrixExpanded` state (default `false`) 추가.
  - 그룹 카드 안의 RotationPreviewMatrix 인스턴스 제거 → 카드 외부 (액션 버튼 위) 별도 collapsible 영역.
  - 헤더 「🔄 회전 미리보기 ▶ 펼쳐서 확인」 — 클릭 시 매트릭스 렌더.
  - 회전 ON 그룹 + 시프트≥1 + 멤버≥1 일 때만 표시.
- 알고리즘/스키마 변경 없음. tsc CallScheduler 영역 에러 0.
- 향후 정리 영역 (선택): 멤버 cfg 펼침 패널 추가 컴팩트, 「시프트 추가」 칩 영역 컴팩트.
