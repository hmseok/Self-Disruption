...
- 2026-05-06 PR-2SS-h-1-fix — 모달 폐기 → 인라인 펼침 (accordion). chip 클릭 = 펼침/접힘 토글. 빠른 입력 한 줄 (시작일~종료일~사유~[+추가]). 시작일 입력 시 종료일 자동 동일.
- 2026-05-08 PR-2SS-h-4 — 매트릭스 (ScheduleGrid) 헤더 영역에 회피일 워커별 행 추가. 외부 cycle 행 패턴 따라 일자별 🛌(승인) / ⏳(대기) 표시. 매니저가 한눈에 "왜 빈 자리지" 답 가능.
