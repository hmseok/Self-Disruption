...
| 2026-05-04 | 21 PR (2A~2Z + 2AA) 누적 반영, 24/365 재구성, 휴가 발급량 시스템 | Claude |

## 6. 본 모듈 강력 규칙 (CLAUDE.md 규칙 22~26 본 모듈 적용)

### 6.1 _docs 갱신 의무 (규칙 22)

새 PR 종료 시 **반드시** 다음 중 1개 이상 갱신:
- `CHANGELOG.md` — 한 줄 이상 (PR 코드 + 한 줄 요약)
- `DATA-MODEL.md` — 마이그레이션 / 모델 변경 시
- `UI-SPEC.md` — 새 페이지/컴포넌트/모드
- `OPERATIONS.md` — 운영 사실 변경
- `SCENARIOS.md` — 페르소나/흐름 변경

**검증**: commit 전 `app/CallScheduler/_docs/` 안 파일 변경 여부 셀프 체크.
없으면 commit 메시지에 "[no-docs-change]" 명시 (정당화).

### 6.2 마이그레이션 검증 (규칙 23)

본 모듈 마이그레이션 SQL 만들 때:
- 사용자에게 적용 안내 즉시 (UI 작업 전)
- API graceful fallback — 테이블 미적용 시 빈 배열 + `_migration_pending: true`
- UI 측 헤더에 ⚠ 마이그레이션 미적용 배너

### 6.3 시드 멱등성 (규칙 24)

본 모듈 시드 INSERT는 **모두** `INSERT IGNORE` 또는 `NOT EXISTS` 가드.
시드 대상 테이블 UNIQUE 키:
- ✅ `cs_shift_slots.code`
- ✅ `cs_workers.name` ← **추가 필요** (현재 없음)
- ✅ `cs_shift_groups.name` ← **추가 필요**

### 6.4 24/365 운영 (규칙 25)

본 모듈은 24/365 콜센터:
- `cs_holidays.exclude_auto` 디폴트 false
- 자동 생성 `skip_holidays` 디폴트 false
- "회사 차원 일괄 휴무" 개념 없음 — 모든 휴무는 직원별

### 6.5 페르소나 사전 점검 (규칙 26)

새 UI/API 만들기 전 `_docs/SCENARIOS.md` 의 페르소나 1/2/3 흐름 점검.
변경 영향 받는 Step 명시 후 작업 진입.

## 7. 본 세션 사고 회고 (재발 방지 학습)

| # | 사고 | 발생 PR | 학습 → 규칙 |
|---|------|--------|-----------|
| 1 | _docs 14 PR 누적 빠뜨림 | PR-2A~2Z | 메인 규칙 22 |
| 2 | cs_holidays/leaves 미적용 UI 500 | PR-2O/2P | 메인 규칙 23 |
| 3 | cs_workers 16→48 중복 | 마이그 적용 시 | 메인 규칙 24 |
| 4 | 24/365 운영 후반 인지 | PR-2P 후 | 메인 규칙 25 |
| 5 | 페르소나 분리 늦게 | PR-2C 무렵 | 메인 규칙 26 |
