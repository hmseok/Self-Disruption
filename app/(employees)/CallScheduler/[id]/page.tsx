...
// PR-2RR-g (2026-05-28) — DistributionDialog (Phase 2 stub) 폐기, DistributeModal (알리고) 단일 사용
// import DistributionDialog from '../components/DistributionDialog'
import DistributeModal from '../_components/DistributeModal'
...
function DetailInner({ id }: { id: string }) {
  ...
  // PR-2RR-g — distOpen 폐기. setSmsOpen 으로 일원화.
  const [drawerOpen, setDrawerOpen] = useState(false)
  const [mode, setMode] = useState<ViewMode>('view')
  const [moreOpen, setMoreOpen] = useState(false)
  const [smsOpen, setSmsOpen] = useState(false)  // CX-KPI-21 — 알리고 SMS 배포
  ...
  // PR-2RR-g — workers 는 더이상 destructure 필요 X (DistributionDialog 폐기). detail.workers 직접 참조.
  const { schedule, kpi, distributions } = detail
  ...
  return (
    <div style={{ padding: '16px 20px', width: '100%' }}>
      ...
      <div style={{
        ...
        {/* MONTH-NAV (2026-05-28): 좌우 화살표 + 드롭다운으로 월 간 이동 */}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', position: 'relative' }}>
          ...
          {/* PR-2RR-g (2026-05-28) — 「⚡ 배포」 = 알리고 SMS 단일 액션.
              기존 「📤 문자 배포」 별도 버튼 + DistributionDialog (Phase 2 stub) 통합.
              사용자 명령: 「배포는 문자는 알리고로 보내주세요. 키는 연결되어있습니다」. */}
          <button
            type="button"
            onClick={() => setSmsOpen(true)}
            disabled={detail.assignments.length === 0}
            style={{
              ...BTN.lg,
              background: detail.assignments.length === 0 ? COLORS.bgGray : COLORS.primary,
              color: detail.assignments.length === 0 ? COLORS.textMuted : '#fff',
              border: 'none',
              cursor: detail.assignments.length === 0 ? 'not-allowed' : 'pointer',
            }}
            title={detail.assignments.length === 0
              ? '배정된 시프트가 없어 발송할 직원이 없습니다'
              : '근무표를 알리고 SMS 로 직원에게 발송 (미리보기 → 검증 → 실발송)'}
          >
            ⚡ 배포 (SMS)
          </button>
      ...
      {/* PR-2RR-g (2026-05-28) — DistributionDialog 렌더 폐기, DistributeModal (알리고 SMS) 단일 사용 */}
      <DistributeModal
        open={smsOpen}
