...
## 2026-05-24 | PR-VISION-17 | 빈도 히트맵 + 문자 발송 (Aligo)

### 사용자 요청
> "통계 그리드 — 많이 나온 횟수 기준 빨강~파랑 톤차이로" / "문자 구현"

### 변경
- `lotto/page.tsx`
  - 번호 통계 — `Ball` 에 빈도 히트맵 색상(`heatColor`): 최다 빨강 ~ 최소 파랑
    핫/콜드 + 45번 그리드 전부 빈도별 톤 적용 (히트맵 시 흰 글씨+그림자로 가독성)
  - 「내 기록」 액션 컬럼에 「📱 문자」 버튼 — 해당 회차 게임을 본인 휴대폰으로 전송
  - 문자 발송 결과 글래스 패널 (Rule 20 — alert 미사용)
- **NEW** `app/api/ride-vision/lotto-sms/route.ts`
  - POST `{ draw_no }` → `profiles.phone`(로그인 본인) 으로 그 회차 게임 SMS
  - 기존 `app/utils/messaging.ts` `sendSMS`(Aligo) 재사용 — "나한테" = 프로필 휴대폰

### 비고
- 프로필에 휴대폰 없으면 "프로필에 휴대폰 없음" 안내. ALIGO_* 환경변수는 기존 SMS 기능과 공유.

### GATE
- ✅ G5 tsc 0건 / G6 lint:harness 새 위반 0건 / Rule 22
...
### 사용자 질문
> "당첨확률 높이려고 과거 추첨 데이터 분석한 게 안 들어갔나?"

→ 과거 출현 빈도는 당첨 확률과 무관함(매 회차 독립 추첨)을 설명. 거짓 「당첨률 ↑」
주장은 넣지 않되, 정직 표기 조건으로 「재미용 번호 통계」 추가.

### 변경
- **NEW** `app/api/ride-vision/lotto-stats/route.ts`
  - `ride_lotto_results` 캐시 전체의 번호별 출현 빈도 집계 (freq[1~45], bonusFreq)
- `lotto/page.tsx` — 3번째 탭 「📊 번호 통계」 신설
  - 정직 배너 — 「독립 추첨 · 출현 빈도는 당첨 확률과 무관」 명시
  - 🔥핫 / ❄️콜드 각 7개, 45번 출현 빈도 그리드, 홀짝·구간 분포
  - 캐시된 회차만 집계 (회차 열람 시 누적)
...
## 2026-05-24 | PR-VISION-14 | UI 큼직·화려 개편 (위젯 2-패널 + 시스템 소개)

### 사용자 요청
> "UI 더 화려하게 / 큼직큼직하게 / 시간 카운트도 / 믿음 가는 시스템 소개"

### 변경
- `lotto/page.tsx`
  - 이번 회차 위젯 → 큼직한 2-패널
    · 패널 A: 제N회(그라디언트 36px) + 카운트다운 블록 4개(일/시/분/초, 28~30px) + 구매 버튼
    · 패널 B: 최근 추첨 결과 — 제N회 + 큰 당첨공 46px (6 + 보너스)
  - 「시스템 소개」 섹션 신설 — RideVision 로또 시스템 / 「믿을 건 로또 뿐」
    4개 기능 카드 (공정 추출 / 동행복권 연동 / 손익 분석 / 운세)
  - 정직 문구 유지: 로또는 운 — 시스템은 추출·기록·확인을 정확히 책임 (당첨확률 과장 X)

### 비고
- 페이지 이름 「믿을 건 로또 뿐」 변경은 메인 세션 담당 (PageTitle/menu-registry — 공통 파일)

### GATE
- ✅ G5 tsc 0건 / G6 lint:harness — cowork-staging-lint 단일 모듈(RideVision) commit

---

## 2026-05-24 | PR-VISION-13 | 동행복권 스타일 위젯 (회차·카운트다운·최근결과)

### 사용자 요청
> 동행복권 메인 스타일 — 이번 회차 / 추첨 예정일 / 남은시간 / 구매링크 / 최근 결과 상단 표출

### 변경
- `lotto/page.tsx` — 추출 탭 상단 「이번 회차 배너」 → 위젯으로 확장
  - 제N회 + 추첨 예정일(토 20:35) + 남은시간 1초 카운트다운(일/시:분:초)
  - 「동행복권 구매 ↗」 외부 링크 (`dhlottery.co.kr/lt645/intro`)
  - 최근 추첨 결과 — 직전 회차(roundInfo.round−1) 당첨번호 6 + 보너스 공
    (`/api/ride-vision/lotto-result` 조회, 실패 시 graceful 미표시)
  - 이번 회차 구매 N/5 표시 유지
- 1등 예상 당첨금은 selectMainInfo 에 안정적 노출이 없어 제외
...
### 사용자 요청
> "구매내역에서 복사해서 (사러) 갈 수 있게" / "삭제 권한은 슈퍼어드민에게"

### 변경
- **수정** `lotto/page.tsx` — 「내 기록」 테이블에 액션 컬럼 추가
  - 「복사」 버튼 — 전 직원, 해당 기록의 6개 번호를 클립보드 복사 (실제 구매용)
  - 「삭제」 버튼 — admin/master 역할에게만 노출 (`getStoredUser().role`)
- **수정** `app/api/ride-vision/lotto-entries/[id]/route.ts`
  - DELETE 를 슈퍼어드민(admin/master) 전용으로 제한 (이전: 본인 기록 누구나)
  - 사유: 손실 추적 무결성 — 직원이 낙첨 기록을 임의 삭제하면 손익 집계 무의미
  - 슈퍼어드민은 모든 직원 기록 삭제 가능 (user_id 조건 제거 — 정정·정리용)
