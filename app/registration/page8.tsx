      ...
      for (let i = 0; i < files.length; i++) {
          ...
          try {
              ...
              const insertPayload = {
                  ...
                  // 🔥 차대번호(VIN)이 없으면 임의값 생성 (중복 에러 방지용)
              ...
              // 👇 [핵심 수정] insert 대신 upsert 사용 (차대번호가 같으면 덮어쓰기)
              const { error: dbError } = await supabase
                  .from('cars')
                  .upsert([insertPayload], { onConflict: 'vin' }) // VIN이 Key

              if (dbError) throw dbError

              setProgress(prev => ({ ...prev, success: prev.success + 1 }))
              setLogs(prev => [`[✅ 성공] ${detectedNumber} - ${detectedModel} (업데이트됨)`, ...prev])
