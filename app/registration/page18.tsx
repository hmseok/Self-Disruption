...
export default function RegistrationListPage() {
  ...
  const [vin, setVin] = useState('') // 차대번호
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          try {
              ...
              setLogs(prev => [`✅ ${detectedModel} (${result.car_number}) 완료`, ...prev])

          } catch (error: any) {
              setProgress(prev => ({ ...prev, fail: prev.fail + 1 }))
              setLogs(prev => [`❌ ${file.name} 실패: ${error.message}`, ...prev])
          }
      }
  ...
  const handleRegister = async () => {
    ...
    if (!vin) return alert('차대번호를 입력해주세요.')
    ...
    if (error) alert('등록 실패: ' + error.message)
        ...
        fetchList();
        setCarNum(''); setVin(''); setSelectedModelName(''); setSelectedTrim(null);
    }
    setCreating(false)
  }
    ...
    <div className="max-w-7xl mx-auto py-12 px-6 bg-gray-50/50 min-h-screen">
       ...
       <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
         <div>
            ...
            <p className="text-gray-500 mt-2 text-sm">AI OCR로 등록증을 분석하여 차량을 자동 등록합니다.</p>
         </div>
         <div className="flex gap-3">
            <label className={`cursor-pointer group flex items-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-xl font-bold hover:bg-blue-700 hover:shadow-lg transition-all transform hover:-translate-y-0.5 ${bulkProcessing ? 'opacity-50 pointer-events-none' : ''}`}>
                <Icons.Upload />
                <span>{bulkProcessing ? '분석 중...' : '등록증 일괄 업로드'}</span>
       ...
       {bulkProcessing && (
         <div className="mb-10 bg-gray-900 rounded-2xl p-6 shadow-2xl ring-4 ring-blue-500/10 overflow-hidden relative">
            ...
            <div className="h-32 overflow-y-auto font-mono text-xs text-gray-300 border-t border-gray-700 pt-2">{logs.map((log, i) => <div key={i}>{log}</div>)}</div>
         </div>
       )}
                        ...
                        <tr key={car.id} onClick={() => router.push(`/registration/${car.id}`)} className="group hover:bg-blue-50/30 transition-colors cursor-pointer">
                            <td className="p-5 pl-8">
                                <div className="w-14 h-10 bg-gray-100 rounded border overflow-hidden">
                                    {car.registration_image_url ? <img src={car.registration_image_url} className="w-full h-full object-cover" /> : <div className="flex items-center justify-center h-full text-gray-300"><Icons.File /></div>}
                                </div>
                            </td>
                            <td className="p-5">
                                <div className="font-black text-gray-900 text-lg">{car.number}</div>
                                <div className="text-gray-500 text-sm font-medium">{car.model}</div>
                            </td>
                            <td className="p-5">
                                <div className="text-gray-900 font-bold">{car.owner_name || '-'}</div>
                                {/* 🔥 [수정됨] 차대번호 전체 표시 (줄임 없음) */}
                                <div className="text-xs text-gray-500 font-mono mt-1 tracking-tight bg-gray-50 inline-block px-1 rounded border border-gray-100">
                                    {car.vin || '(차대번호 없음)'}
                                </div>
                            </td>
                            <td className="p-5">
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8 space-y-5">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">차량 번호</label>
                        <input className="w-full p-3 border rounded-xl font-bold" placeholder="12가 3456" value={carNum} onChange={e=>setCarNum(e.target.value)} />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">차대 번호 (필수)</label>
                        <input className="w-full p-3 border rounded-xl font-mono uppercase" placeholder="VIN 입력" value={vin} onChange={e=>setVin(e.target.value)} />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select className="w-full p-3 border rounded-xl" onChange={e=>setSelectedModelName(e.target.value)} defaultValue="">
                    ...
                    <div>
                        ...
                        <select className="w-full p-3 border rounded-xl" onChange={e=>setSelectedTrim(standardCodes.find(s => s.id === Number(e.target.value)))} disabled={!selectedModelName} defaultValue="">
                            <option value="" disabled>선택</option>
