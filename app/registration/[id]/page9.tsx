...
export default function RegistrationPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        reader.onload = async () => {
          ...
          if (!ocrData.error) {
              console.log("🔍 AI 원본:", ocrData);

              // 🚀 [수정] 날짜/금액 청소 적용하여 State 업데이트
              setCar(prev => ({
                  ...prev,
                  ...
                  // 👇 날짜 청소 (2025.07.04 -> 2025-07-04)
                  registration_date: cleanDate(ocrData.registration_date) || prev.registration_date,
                  inspection_end_date: cleanDate(ocrData.inspection_end_date) || prev.inspection_end_date,
                  vehicle_age_expiry: cleanDate(ocrData.vehicle_age_expiry) || prev.vehicle_age_expiry,

                  location: ocrData.location || prev.location,

                  // 👇 숫자 청소 (콤마 제거)
                  capacity: cleanNumber(ocrData.capacity) || prev.capacity,
                  displacement: cleanNumber(ocrData.displacement) || prev.displacement,
                  purchase_price: cleanNumber(ocrData.purchase_price) || prev.purchase_price,
              ...
              try {
                  if (ocrData.model) {
                      ...
                      if (finalModelCode) setCar(prev => ({ ...prev, model_code: finalModelCode }))
                  }
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('cars').update({
        ...
        capacity: cleanNumber(car.capacity),
        displacement: cleanNumber(car.displacement),
    ...
    if (error) {
        console.error("DB 상세 에러:", error) // 이제 에러 내용이 보일 겁니다
