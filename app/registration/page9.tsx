...
export default function RegistrationListPage() {
  ...
  // 🚀 [핵심] 대량 등록 핸들러 (이미지 URL 수정 + 트림 자동 매칭)
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      const files = e.target.files
      if (!files || files.length === 0) return

      if (!confirm(`총 ${files.length}장의 등록증을 분석하여 등록하시겠습니까?`)) return
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          try {
              // 1. 이미지 업로드 (경로에 타임스탬프 추가하여 캐싱 방지)
              ...
              // 🖼️ [이미지 URL] getPublicUrl 결과를 변수에 확실히 저장
              ...
              if (!response.ok) throw new Error('AI 서버 연결 실패');
              ...
              // 🆕 4. [트림 자동 매칭] 모델명으로 DB 뒤져서 기본 트림 찾기
              let finalModelCode = ''
              let matchedModelId = null;
              let finalPrice = cleanNumber(result.purchase_price) || 0;
              let finalModelName = detectedModel; // 기본은 AI가 읽은 값
              ...
              if (detectedModel && detectedModel !== '미확인 모델') {
                  ...
                  const { data: existingModel } = await supabase
                    .from('car_code_models') // 🚨 기존 vehicle_model_codes 대신 car_code_models 사용 (있다고 가정)
                    .select('*')
                    .or(`model_name.ilike.%${cleanName}%, code.eq.${cleanName}`) // 유사 검색
                    .limit(1)
                    .single();

                  if (existingModel) {
                      matchedModelId = existingModel.id;
                      finalModelName = `${existingModel.brand} ${existingModel.model_name}`; // DB 표준명으로 교체

                      // B. 해당 모델의 '기본 트림(최저가)' 찾기
                      const { data: basicTrim } = await supabase
                          .from('car_code_trims')
                          .select('*')
                          .eq('model_id', existingModel.id)
                          .order('price', { ascending: true }) // 가격 오름차순
                          .limit(1)
                          .single();

                      if (basicTrim) {
                          // 트림 정보가 있으면 그걸로 덮어쓰기 (가격도 트림 가격이 더 정확할 수 있음)
                          finalModelName = `${finalModelName} ${basicTrim.trim_name}`;
                          if (finalPrice === 0) finalPrice = basicTrim.price; // AI가 가격 못 읽었으면 기본값 사용
                          console.log(`✨ 기본 트림 매칭 성공: ${basicTrim.trim_name}`);
                      }
                  } else {
                      // 모델이 없으면 코드 테이블에 신규 생성 (선택 사항)
                      // 여기서는 생략하고 그냥 텍스트로 저장
                  }
              }
              ...
              const insertPayload = {
                  ...
                  brand: '기타', // 나중에 모델 매칭되면 업데이트
                  model: finalModelName, // AI값 또는 매칭된 표준명+트림
                  model_code: finalModelCode,
                  vin: result.vin || `NO-VIN-${Date.now()}-${i}`,
                  owner_name: result.owner_name || '',
                  location: result.location || result.address || '',

                  purchase_price: finalPrice, // 0원 방지 로직 적용됨
                  ...
                  registration_image_url: finalPublicUrl, // 🖼️ 생성한 URL 삽입
                  status: 'available',
                  notes: result.notes || ''
              };
              ...
              const { error: dbError } = await supabase
                  .from('cars')
                  .upsert([insertPayload], { onConflict: 'vin' })

              if (dbError) throw dbError

              setProgress(prev => ({ ...prev, success: prev.success + 1 }))
              setLogs(prev => [`[✅ 성공] ${detectedNumber} - ${finalModelName}`, ...prev])
              ...
              <tr key={car.id} onClick={() => router.push(`/registration/${car.id}`)} className="hover:bg-gray-50 cursor-pointer">
                ...
                <td className="p-4 text-center">
                  {car.registration_image_url ? (
                     <a href={car.registration_image_url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()}>
                        ✅
                     </a>
