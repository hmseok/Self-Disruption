...
import { createClientComponentClient } from '@supabase/auth-helpers-nextjs' // 경로 확인
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white rounded-3xl shadow-sm border border-gray-200 min-h-[600px] flex flex-col">
          ...
          <div className="p-8 flex-1">
            ...
            {/* 4. 지입/투자 탭 (경로 수정됨: /jiip/[id]) */}
            {activeTab === 'jiip' && (
              <div className="flex flex-col items-center justify-center h-full py-10 animate-fade-in">
                <div className="bg-orange-50 p-6 rounded-full mb-4">
                  <span className="text-4xl">🤝</span>
                </div>
                <h3 className="text-xl font-bold text-gray-800 mb-2">지입/투자 관리</h3>
                <p className="text-gray-500 mb-8 text-center max-w-sm">
                  지입 차주 계약 정보 및 투자자 배당금 정산 내역을 관리하는 페이지입니다.
                </p>
                <button
                  onClick={() => router.push(`/jiip/${carId}`)}
                  className="bg-orange-600 text-white px-8 py-4 rounded-xl font-bold text-lg shadow-lg hover:bg-orange-700 transition-transform hover:-translate-y-1"
                >
                  지입 상세 페이지로 이동 →
                </button>
              </div>
