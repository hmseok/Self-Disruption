...
const STATUS_LABELS: Record<string, { label: string; bg: string }> = {
  active: { label: '운용중', bg: 'bg-green-100 text-green-700' },
...
export default function GeneralInvestDetail() {
  ...
  const fetchDetail = async () => {
    ...
    setItem({ ...data, investor_address: data.investor_address || '', investor_address_detail: data.investor_address_detail || '', status: data.status || 'active' })
  ...
  const generateSchedule = async () => {
    if (!confirm('결제 스케줄을 (재)생성합니다.')) return
  ...
  const handleMoneyChange = (val: string) => { const n = Number(val.replace(/,/g, '')); if (!isNaN(n)) setItem((prev: any) => ({ ...prev, invest_amount: n })) }
          ...
          <div>
            ...
            {!isNew && (
              <div className="flex items-center gap-3 mt-2">
                ...
                <span className="text-xs font-bold text-steel-600 bg-steel-50 px-2 py-0.5 rounded">연 {item.interest_rate}% · 월 {monthlyInterest.toLocaleString()}원</span>
              </div>
            )}
          </div>
          {!isNew && (
            <div className="flex gap-2">
              {item.status === 'active' && (
                <select value="" onChange={e => { if (e.target.value) handleStatusChange(e.target.value) }} disabled={changingStatus}
              ...
              {item.status === 'expired' && (
                <button onClick={() => handleStatusChange('renewed')} disabled={changingStatus}
                  className="text-sm bg-blue-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-blue-700">갱신</button>
              )}
      ...
      <div className="flex gap-1.5 mb-6">
        {TABS.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`px-4 py-2.5 rounded-xl font-bold text-sm transition-all ${activeTab === tab.key ? 'bg-steel-900 text-white' : 'bg-white text-gray-500 border border-gray-200 hover:bg-gray-50'}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'info' && (
        <div className="space-y-6 bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="space-y-4">
            <h3 className="font-bold text-base text-gray-900">1. 투자자 정보</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-xs font-bold text-gray-500 mb-1">성명/법인명</label><input className="w-full border p-3 rounded-xl font-bold text-sm" value={item.investor_name} onChange={e => setItem({ ...item, investor_name: e.target.value })} /></div>
              <div><label className="block text-xs font-bold text-gray-500 mb-1">연락처</label><input className="w-full border p-3 rounded-xl text-sm" placeholder="010-0000-0000" value={item.investor_phone} onChange={e => setItem({ ...item, investor_phone: formatPhone(e.target.value) })} maxLength={13} /></div>
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-gray-500 mb-1">주소</label>
                <div className="flex gap-2 mb-2">
                  <input className="w-full border p-3 rounded-xl bg-gray-50 text-sm" value={item.investor_address} readOnly placeholder="주소 검색 버튼을 눌러주세요" />
                  <button onClick={() => open({ onComplete: handleAddress })} className="bg-gray-800 text-white px-4 rounded-xl font-bold text-sm whitespace-nowrap">주소검색</button>
                </div>
                <input className="w-full border p-3 rounded-xl text-sm" placeholder="상세 주소 입력" value={item.investor_address_detail} onChange={e => setItem({ ...item, investor_address_detail: e.target.value })} />
              </div>
            </div>
          </div>
          ...
          {/* 투자 조건 */}
          <div className="space-y-4">
            <h3 className="font-bold text-base text-gray-900">2. 투자 조건</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-5 rounded-xl border border-slate-200">
              <div>
                <label className="block text-xs font-bold text-steel-600 mb-1">투자 약정금</label>
                <div className="relative">
                  <input className="w-full border border-steel-100 p-3 pr-10 rounded-xl text-right font-black text-xl text-gray-900 focus:border-steel-500 outline-none" value={item.invest_amount ? Number(item.invest_amount).toLocaleString() : ''} onChange={e => handleMoneyChange(e.target.value)} placeholder="0" />
              ...
              {!isNew && (
                <div>
                  ...
                  <div className={`w-full border-2 p-3 rounded-xl text-right font-black text-xl flex justify-end items-center gap-1 ${realDepositTotal >= item.invest_amount && item.invest_amount > 0 ? 'border-green-400 bg-green-50 text-green-700' : 'border-red-200 bg-white text-red-600'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div><label className="block text-xs font-bold text-green-600 mb-1">연 수익률(%)</label><input type="number" className="w-full border p-3 rounded-xl text-right font-bold text-sm" value={item.interest_rate} onChange={e => setItem({ ...item, interest_rate: e.target.value })} /></div>
              <div><label className="block text-xs font-bold text-gray-500 mb-1">이자 지급일</label><input type="number" className="w-full border p-3 rounded-xl text-right text-sm" placeholder="10" value={item.payment_day} onChange={e => setItem({ ...item, payment_day: e.target.value })} /></div>
              <div><label className="block text-xs font-bold text-gray-500 mb-1">계약 시작일</label><input type="date" className="w-full border p-3 rounded-xl text-sm" value={item.contract_start_date} onChange={e => setItem({ ...item, contract_start_date: e.target.value })} /></div>
          ...
          {/* 계좌 정보 */}
          <div className="space-y-4">
            <h3 className="font-bold text-base text-gray-900">3. 입금 계좌 정보</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div><label className="block text-xs font-bold text-gray-500 mb-1">은행명</label><select className="w-full border p-3 rounded-xl bg-white text-sm" value={item.bank_name} onChange={e => setItem({ ...item, bank_name: e.target.value })}>{KOREAN_BANKS.map(b => <option key={b} value={b}>{b}</option>)}</select></div>
              <div className="md:col-span-2"><label className="block text-xs font-bold text-gray-500 mb-1">계좌번호</label><input className="w-full border p-3 rounded-xl text-sm" value={item.account_number} onChange={e => setItem({ ...item, account_number: formatAccount(e.target.value) })} /></div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="space-y-2 mb-4">
              {(sendChannel === 'email' || sendChannel === 'both') && (
                <input type="email" className="w-full border p-3 rounded-xl text-sm" placeholder="수신자 이메일" value={sendingEmail} onChange={e => setSendingEmail(e.target.value)} />
              )}
              {(sendChannel === 'kakao' || sendChannel === 'both') && (
                <input type="tel" className="w-full border p-3 rounded-xl text-sm" placeholder="수신자 휴대폰 (예: 010-1234-5678)" value={sendingPhone} onChange={e => setSendingPhone(e.target.value)} />
              )}
            </div>
                ...
                {sendingLogs.slice(0, 10).map((log: any) => (
                  <div key={log.id} className="px-4 py-2.5 flex items-center justify-between text-sm">
                    <div className="flex items-center gap-3">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded ${log.status === 'sent' ? 'bg-blue-100 text-blue-700' : log.status === 'signed' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {log.status === 'sent' ? '발송' : log.status === 'signed' ? '서명' : '실패'}
                      </span>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
              <button onClick={() => setShowSignPad(true)} className="bg-white text-steel-900 py-3.5 rounded-xl font-bold text-sm shadow-sm hover:shadow-md border border-slate-200 flex items-center justify-center gap-2 transition-all">직접 서명</button>
              <button onClick={() => setShowPreview(true)} className="bg-white text-gray-700 py-3.5 rounded-xl font-bold text-sm shadow-sm hover:shadow-md border border-slate-200 flex items-center justify-center gap-2 transition-all">미리보기/인쇄</button>
            </div>
        ...
        <div className="space-y-6">
          {paymentSummary && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="bg-white p-4 rounded-xl border border-slate-200"><p className="text-xs font-bold text-slate-400">총 스케줄</p><p className="text-xl font-black text-slate-900">{paymentSummary.total_months}<span className="text-sm font-bold text-slate-400">개월</span></p></div>
              <div className="bg-white p-4 rounded-xl border border-slate-200"><p className="text-xs font-bold text-slate-400">완료</p><p className="text-xl font-black text-green-600">{paymentSummary.completed}<span className="text-sm font-bold text-slate-400">건</span></p></div>
              <div className="bg-white p-4 rounded-xl border border-slate-200"><p className="text-xs font-bold text-slate-400">예상 총 이자</p><p className="text-lg font-black text-slate-900">{paymentSummary.total_expected?.toLocaleString()}<span className="text-sm font-bold text-slate-400">원</span></p></div>
              <div className="bg-white p-4 rounded-xl border border-slate-200"><p className="text-xs font-bold text-slate-400">미지급</p><p className={`text-lg font-black ${paymentSummary.balance > 0 ? 'text-red-600' : 'text-green-600'}`}>{paymentSummary.balance?.toLocaleString()}<span className="text-sm font-bold text-slate-400">원</span></p></div>
            </div>
          )}

          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-base text-slate-900">월별 이자 지급 스케줄</h3>
              <button onClick={generateSchedule} disabled={generatingSchedule}
                className="px-4 py-2 bg-steel-600 text-white rounded-xl font-bold text-sm hover:bg-steel-700 disabled:bg-slate-300">
            ...
            {paymentSchedules.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      <th className="p-3 text-left text-xs font-bold text-slate-400">지급 예정일</th>
                      <th className="p-3 text-right text-xs font-bold text-slate-400">예상 이자</th>
                      <th className="p-3 text-right text-xs font-bold text-slate-400">실제 지급</th>
                      ...
                      return (
                        <tr key={s.id} className={`border-b border-slate-50 ${isOverdue ? 'bg-red-50/50' : ''}`}>
                          ...
                          <td className="p-3 text-center">
                            <span className={`text-xs font-bold px-2 py-0.5 rounded ${s.status === 'completed' ? 'bg-green-100 text-green-700' : isOverdue ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-500'}`}>
              ...
              <div className="text-center text-slate-400 p-8 border-2 border-dashed border-slate-200 rounded-xl">
                <p className="font-bold text-sm">이자 지급 스케줄이 없습니다.</p>
              ...
              <div className="divide-y divide-slate-100">
                {paymentTransactions.map((t: any) => (
                  <div key={t.id} className="py-3 flex justify-between items-center">
                    <div><p className="text-sm font-bold text-slate-700">{t.description || '입금'}</p><p className="text-xs text-slate-400">{formatDateTime(t.created_at)}</p></div>
      ...
      {showSignPad && (
        <div className="fixed inset-0 z-[9999] bg-gray-100 flex flex-col">
          <div className="bg-steel-900 text-white p-4 flex justify-between items-center shadow-md z-10">
            <div><h3 className="font-bold text-lg">관리자 직접 서명</h3><p className="text-xs text-steel-200">내용을 확인하고 서명해주세요.</p></div>
          ...
          <div className="flex-1 overflow-y-auto bg-gray-500 p-4">
            <div className="flex justify-center"><div className="bg-white shadow-xl rounded-sm overflow-hidden min-h-[500px] mb-40 shrink-0" style={{ width: '100%', maxWidth: '210mm' }}><GeneralContract data={previewData} mode="mobile" /></div></div>
          </div>
          <div className="bg-white p-4 shadow-[0_-4px_15px_rgba(0,0,0,0.1)] z-20 pb-8 rounded-t-2xl fixed bottom-0 left-0 right-0">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={saveSignature} disabled={uploading} className="flex-[2] bg-steel-600 py-4 rounded-xl font-bold text-white shadow-lg">{uploading ? '처리 중...' : '서명 완료'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 z-[9999] flex flex-col items-center justify-center p-4">
          <div className="bg-gray-100 w-full max-w-5xl rounded-xl overflow-hidden flex flex-col h-[90vh] shadow-2xl">
            <div className="p-4 bg-white border-b flex justify-between">
              ...
              <div className="flex gap-2"><button onClick={() => window.print()} className="bg-black text-white px-3 rounded font-bold text-sm">인쇄</button><button onClick={() => setShowPreview(false)} className="bg-gray-200 px-3 rounded font-bold text-sm">닫기</button></div>
            </div>
