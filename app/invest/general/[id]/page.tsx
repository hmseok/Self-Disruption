...
export default function GeneralInvestDetail() {
  ...
  const [item, setItem] = useState<any>({
    ...
    investor_address: '',         // 🏠 기본 주소 (검색)
    investor_address_detail: '',  // 🏢 상세 주소 (직접 입력)
    bank_name: 'KB국민은행', account_number: '', account_holder: '',
    invest_amount: 0, interest_rate: 12, payment_day: 10, // 기본값: 12%, 10일
  ...
  const handleAddress = (data: any) => {
    ...
    setItem({...item, investor_address: full}) // 상세 주소는 그대로 유지
  }
  ...
  const formatAccount = (v: string) => v.replace(/[^0-9-]/g, "") // 숫자와 하이픈만 허용
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-6 pb-32">
        ...
        <div className="space-y-8 bg-white p-8 rounded-3xl shadow-sm border border-gray-200 mb-8">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">성명/법인명</label>
                        <input className="w-full border p-3 rounded-xl font-bold" value={item.investor_name} onChange={e=>setItem({...item, investor_name:e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">연락처</label>
                        <input className="w-full border p-3 rounded-xl" placeholder="010-0000-0000" value={item.investor_phone} onChange={e=>setItem({...item, investor_phone:formatPhone(e.target.value)})} maxLength={13} />
                    </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label className="block text-xs font-bold text-blue-600 mb-1">투자 원금 (KRW)</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-green-600 mb-1">연 수익률(%)</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">이자 지급일</label>
            ...
            {/* 3. 계좌 정보 */}
            <div className="space-y-4">
                <h3 className="font-bold text-lg text-gray-900 border-b pb-2 pt-2">3. 입금 계좌 (이자 수령용)</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                    ...
                    <div className="md:col-span-2">
                        ...
                        <input className="w-full border p-3 rounded-xl" value={item.account_number} onChange={e=>setItem({...item, account_number:formatAccount(e.target.value)})} placeholder="하이픈(-) 없이 입력" />
                    </div>
                </div>
            </div>
        ...
        {/* 2️⃣ 서명 및 파일 관리 (하단 배치, 수정 모드일 때만) */}
        {!isNew && (
