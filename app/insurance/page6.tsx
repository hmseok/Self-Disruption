  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          try {
              ...
              if (!detectedVin || detectedVin.length < 10) throw new Error(`문서에서 차대번호(VIN)를 찾을 수 없습니다.`);

              // DB 매칭
              const { data: carData } = await supabase.from('cars').select('id, number, brand').ilike('vin', `%${detectedVin.slice(-6)}`).maybeSingle();
              if (!carData) throw new Error(`등록되지 않은 차대번호입니다: ${detectedVin}`);
              ...
              if (insertError) {
                  console.error("DB Insert Error:", insertError); // 콘솔에 에러 상세 출력
                  throw new Error(`DB 저장 실패: ${insertError.message}`);
              }

              setProgress(prev => ({ ...prev, success: prev.success + 1 }))
              setLogs(prev => [`✅ [${isCertificate?'증명서':'청약서'}] ${carData.number} 저장완료`, ...prev])
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        ...
        <div className="flex gap-3">
            <label className={`cursor-pointer group flex items-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-xl font-bold hover:bg-blue-700 hover:shadow-lg transition-all transform hover:-translate-y-0.5 ${bulkProcessing ? 'opacity-50 pointer-events-none' : ''}`}>
                <Icons.Upload />
                <span>{bulkProcessing ? '분석 중...' : '증권 AI 등록 (PDF가능)'}</span>
