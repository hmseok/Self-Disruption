...
export default function InsuranceListPage() {
  ...
  const fetchList = async () => {
    const { data, error } = await supabase
      .from('cars')
      .select(`id, number, model, brand, insurance_contracts (id, company, end_date, premium, status)`)
      .order('created_at', { ascending: false })

    if (error) {
        console.error("🔥 리스트 로딩 실패:", error.message);
        return;
    }
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!confirm(`총 ${files.length}개의 파일을 분석합니다.\n(PDF, JPG, PNG 지원)`)) return
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          try {
              ...
              // 차량 매칭
              const carNum = result.car_number?.replace(/\s+/g, '');
              if (!carNum) throw new Error("차량번호를 찾을 수 없습니다.");

              const { data: carData } = await supabase.from('cars').select('id').eq('number', carNum).maybeSingle();
              if (!carData) throw new Error(`미등록 차량: ${carNum}`);
              ...
              setLogs(prev => [`✅ [성공] ${carNum} (${isPdf ? 'PDF' : 'IMG'})`, ...prev])
    ...
    <div className="max-w-7xl mx-auto py-12 px-6 bg-gray-50/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
            ...
            <p className="text-gray-500 mt-2 text-sm">청약서(PDF/이미지)를 업로드하면 AI가 자동 매칭합니다.</p>
        </div>
      ...
      {bulkProcessing && (
         <div className="mb-10 bg-gray-900 rounded-2xl p-6 shadow-2xl ring-4 ring-blue-500/10 overflow-hidden relative">
            <div className="flex justify-between items-end mb-4 relative z-10 text-white">
                <div className="flex items-center gap-3"><span className="animate-spin text-xl">⚙️</span><span className="font-bold">AI 문서 처리 중...</span></div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {list.length === 0 && <tr><td colSpan={6} className="p-20 text-center text-gray-400">등록된 차량이 없습니다.</td></tr>}
          </tbody>
        </table>
      </div>
