...
export default function InsuranceListPage() {
  ...
  // 🚀 [AI 업로드] 문서 종류(청약서 vs 증명서) 자동 분류
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      const files = e.target.files
      if (!files?.length) return
      if (!confirm(`총 ${files.length}건을 분석합니다.\nAI가 '청약서'와 '가입증명서'를 구분하여 저장합니다.`)) return
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          try {
              ...
              // 🔥 5. 문서 타입에 따라 저장 위치 결정
              const isCertificate = result.doc_type === 'certificate'; // AI가 'certificate'라고 했는가?
              ...
              // ✅ 타입별 파일 URL 저장
              if (isCertificate) {
                  payload.certificate_url = urlData.publicUrl; // 가입증명서 칸에 저장
                  // 청약서 칸은 건드리지 않음 (기존 데이터 유지)
              } else {
                  payload.application_form_url = urlData.publicUrl; // 청약서 칸에 저장
                  // default는 청약서로 취급
              }

              // 이미 등록된 보험 계약이 있는지 확인 (같은 차, 같은 만기일 기준 등)
              // 여기서는 편의상 upsert 또는 insert. 기존에는 insert 였으나,
              // 증명서 추가 업로드를 고려한다면 update가 맞을 수 있음.
              // 하지만 "일괄 등록" 특성상 새로운 계약 생성(insert)으로 유지하되,
              // 같은 VIN에 대해 '미가입' 상태인 계약을 찾아 업데이트하거나 새로 만듭니다.

              // 현재는 무조건 신규 Insert (가장 안전)
              const { error: insertError } = await supabase.from('insurance_contracts').insert([payload]);

              if (insertError) throw insertError;

              setProgress(prev => ({ ...prev, success: prev.success + 1 }))
              setLogs(prev => [`✅ [${isCertificate ? '증명서' : '청약서'}] ${carData.number} 등록`, ...prev])
    ...
    <div className="max-w-7xl mx-auto py-12 px-6 bg-gray-50/50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
            ...
            <p className="text-gray-500 mt-2 text-sm">청약서/증권을 업로드하면 AI가 <b>종류(청약서/증명서)를 구분</b>하여 자동 등록합니다.</p>
        </div>
        <div className="flex gap-3">
            <label className={`cursor-pointer group flex items-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-xl font-bold hover:bg-blue-700 hover:shadow-lg transition-all transform hover:-translate-y-0.5 ${bulkProcessing ? 'opacity-50 pointer-events-none' : ''}`}>
                <Icons.Upload />
                <span>{bulkProcessing ? '문서 분류 중...' : '증권 AI 등록 (PDF가능)'}</span>
