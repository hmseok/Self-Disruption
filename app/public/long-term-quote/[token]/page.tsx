...
// ═══════════════════════════════════════════════════════════════════
// /public/long-term-quote/[token]
//   장기렌트 견적 공개 페이지 (PR-Q1)
//
// 사용자 명시: 영업이 견적 발송 후 카톡/이메일로 링크 전달 → 고객 무인증 조회.
// - share_token 으로 진입, views++ 자동 갱신 (서버)
// - PDF: window.print() + @media print 폴리실
// - 민감 필드 노출 X (owner_id, customer_phone 등은 서버에서 이미 필터)
// ═══════════════════════════════════════════════════════════════════
...
export default function PublicLongTermQuotePage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch(`/api/public/long-term-quotes/${token}`)
      ...
      <div style={{ minHeight: '100vh', padding: '40px 20px' }}>
        ...
        <div className="ltq-sheet" style={{
          ...
          <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {/* 차량 정보 */}
            <section>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#3b6eb5', marginBottom: 8, letterSpacing: 0.6 }}>차량 정보 · {data.contract_type === '신차구입' ? '🆕 신차 구입 계약' : '🚗 기존 차량 계약'}</div>
              <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
                <tbody>
                  {data.vehicle_car_number ? (
                    <tr>
                      ...
                      <td style={cellValue}>{[data.vehicle_brand, data.vehicle_model].filter(Boolean).join(' ') || '-'}</td>
                    </tr>
                  ) : null}
                  {data.vehicle_spec ? (
                    <tr>
                      <td style={cellLabel}>예정 차종</td>
                      <td style={{ ...cellValue, gridColumn: 'span 3' }} colSpan={3}><strong>{data.vehicle_spec}</strong></td>
                    </tr>
                  ) : null}
                </tbody>
              </table>
            ...
            <section style={{ border: '2px solid #3b6eb5', borderRadius: 10, padding: 18, background: 'linear-gradient(135deg, #f4f8fd 0%, #ebf1f9 100%)' }}>
              ...
              <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px dashed rgba(0,0,0,0.1)', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, fontSize: 12 }}>
                ...
                <div>
                  <div style={{ color: '#64748b', fontWeight: 600, fontSize: 11 }}>초도 납입금 합계</div>
