...
// PATCH /api/contract_special_terms/[id]
export async function PATCH(
  ...
  try {
    ...
    await prisma.$executeRaw`
      UPDATE contract_special_terms SET
        updated_at = NOW()
      WHERE id = ${id}
    `

    const updated = await prisma.$queryRaw<any[]>`SELECT * FROM contract_special_terms WHERE id = ${id} LIMIT 1`
...
// DELETE /api/contract_special_terms/[id]
export async function DELETE(
  ...
  try {
    ...
    await prisma.$executeRaw`
      DELETE FROM contract_special_terms WHERE id = ${id}
    `
