...
export async function GET(request: NextRequest) {
  try {
    ...
    let data: any[]
    if (quoteId) {
      data = await prisma.$queryRaw<any[]>`SELECT * FROM customer_signatures WHERE quote_id = ${quoteId}`
    } else {
      data = await prisma.$queryRaw<any[]>`SELECT * FROM customer_signatures ORDER BY created_at DESC LIMIT 500`
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fields = ['quote_id', 'customer_id', 'signature_data', 'signed_at']
    const cols = ['id', ...fields.filter(f => body[f] !== undefined)]
    const vals = [id, ...fields.filter(f => body[f] !== undefined).map(f => body[f] || null)]

    await prisma.$executeRawUnsafe(
      `INSERT INTO customer_signatures (${cols.join(', ')}, created_at, updated_at) VALUES (${cols.map(() => '?').join(', ')}, NOW(), NOW())`,
      ...vals
    )

    const created = await prisma.$queryRaw<any[]>`SELECT * FROM customer_signatures WHERE id = ${id} LIMIT 1`
