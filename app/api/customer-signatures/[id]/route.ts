...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await prisma.$queryRaw<any[]>`SELECT * FROM customer_signatures WHERE id = ${params.id} LIMIT 1`
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$executeRaw`DELETE FROM customer_signatures WHERE id = ${params.id}`
