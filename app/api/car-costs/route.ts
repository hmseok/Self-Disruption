  ...
  try {
    ...
    if (carId) {
      data = await prisma.$queryRawUnsafe<any[]>(
        `SELECT * FROM car_costs WHERE car_id = ? ORDER BY created_at DESC LIMIT 500`,
        carId
      )
    } else {
      data = await prisma.$queryRaw<any[]>`SELECT * FROM car_costs ORDER BY created_at DESC LIMIT 500`
    }
