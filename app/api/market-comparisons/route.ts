...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await prisma.$queryRaw<any[]>`SELECT * FROM market_comparisons ORDER BY created_at DESC LIMIT 500`
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fields = ['competitor_name', 'model_name', 'price', 'terms', 'notes']
    const cols = ['id', ...fields.filter(f => body[f] !== undefined)]
    const vals = [id, ...fields.filter(f => body[f] !== undefined).map(f => body[f] || null)]

    await prisma.$executeRawUnsafe(
      `INSERT INTO market_comparisons (${cols.join(', ')}, created_at, updated_at) VALUES (${cols.map(() => '?').join(', ')}, NOW(), NOW())`,
      ...vals
    )

    const created = await prisma.$queryRaw<any[]>`SELECT * FROM market_comparisons WHERE id = ${id} LIMIT 1`
