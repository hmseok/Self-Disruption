...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await prisma.$queryRaw<any[]>`SELECT * FROM market_comparisons WHERE id = ${params.id} LIMIT 1`
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$executeRawUnsafe(
      `UPDATE market_comparisons SET ${updates.join(', ')}, updated_at = NOW() WHERE id = ?`,
      ...values
    )

    const updated = await prisma.$queryRaw<any[]>`SELECT * FROM market_comparisons WHERE id = ${params.id} LIMIT 1`
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$executeRaw`DELETE FROM market_comparisons WHERE id = ${params.id}`
