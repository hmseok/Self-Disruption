...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await prisma.$queryRaw<any[]>`SELECT * FROM lotte_rentcar_db WHERE id = ${params.id} LIMIT 1`
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$executeRawUnsafe(
      `UPDATE lotte_rentcar_db SET ${updates.join(', ')} WHERE id = ?`,
      ...values
    )

    const updated = await prisma.$queryRaw<any[]>`SELECT * FROM lotte_rentcar_db WHERE id = ${params.id} LIMIT 1`
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$executeRaw`DELETE FROM lotte_rentcar_db WHERE id = ${params.id}`
