...
export async function GET(req: NextRequest) {
  try {
    ...
    // 동적 컬럼 감지
    const [accResult, rentResult] = await Promise.all([
    ...
    // 조합된 SELECT
    const selectParts = [accResult.select, rentResult.select].filter(Boolean).join(', ');

    // JOIN 조건
    const joinClause = rentResult.select
