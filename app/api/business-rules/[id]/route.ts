...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await prisma.$queryRaw<any[]>`SELECT * FROM business_rules WHERE id = ${params.id} LIMIT 1`
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await prisma.$queryRaw<any[]>`SELECT * FROM business_rules WHERE id = ${params.id} LIMIT 1`
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$executeRaw`DELETE FROM business_rules WHERE id = ${params.id}`
