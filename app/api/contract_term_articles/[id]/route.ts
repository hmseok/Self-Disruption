...
// PATCH /api/contract_term_articles/[id]
export async function PATCH(
  ...
  try {
    ...
    await prisma.$executeRaw`
      UPDATE contract_term_articles SET
        updated_at = NOW()
      WHERE id = ${id}
    `

    const updated = await prisma.$queryRaw<any[]>`SELECT * FROM contract_term_articles WHERE id = ${id} LIMIT 1`
...
// DELETE /api/contract_term_articles/[id]
export async function DELETE(
  ...
  try {
    ...
    await prisma.$executeRaw`
      DELETE FROM contract_term_articles WHERE id = ${id}
    `
