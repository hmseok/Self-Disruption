'use client'

import { useEffect } from 'react'
import { useRouter } from 'next/navigation'

// ============================================
// /admin/payroll → /finance/payroll-ops 로 이전 (2026-05-05 PR-A4)
// 「급여 관리」 = 「급여 운영」 (재무/경영 그룹 안)
// 「급여설정」 입력은 /hr/people 모달의 § 급여 설정 탭으로 이전
// ============================================

export default function PayrollRedirect() {
  const router = useRouter()
  useEffect(() => { router.replace('/finance/payroll-ops') }, [router])
  return (
    <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 32, height: 32, border: '3px solid rgba(0,0,0,0.06)', borderTopColor: '#3b82f6', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 12px' }} />
        <p style={{ fontSize: 13, color: '#94a3b8' }}>급여 운영으로 이동 중...</p>
        <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
      </div>
    </div>
  )
}
