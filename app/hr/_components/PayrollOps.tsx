...
// PayrollOps — 급여 운영 컴포넌트 (PR-B7, 2026-05-06)
// /hr 페이지 5번째 탭 안에서 inline 렌더 (사이드바 1메뉴 진짜 통합)
// 운영 4탭: 대장 / 식대 / 프리랜서 지급 / 분석
// 「급여설정」 탭은 폐기 — 마스터 입력은 /hr 모달의 § 급여 설정 탭에서
...
export default function PayrollOps() {
  ...
  // ── 탭 정의 — 운영 영역만 (급여설정은 /hr/people 로 이전) ──
  const TABS: { key: Tab; label: string; count?: number }[] = [
    { key: 'ledger', label: '급여대장', count: payslips.length },
    { key: 'meals', label: '식대/실비' },
    { key: 'freelancers', label: '프리랜서 지급', count: freelancers.length },
  ...
  return (
    <>
      {/* PR-B7 — /hr 의 5번째 탭 안에서 inline 렌더 (page-bg / max-w 제거) */}
      {/* 부모 /hr 가 5탭 nav 그림 — 본 컴포넌트는 sub-탭 (대장/식대/프리랜서지급/분석)만 */}
      <DcToolbar
        search=""
        onSearchChange={() => {}}
        placeholder="급여 검색..."
      ...
      {/* ══════════ 탭2: 급여설정 — PR-A3 폐기 (입력은 /hr/people 의 모달) ══════════ */}
      {false && tab === 'settings' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    <tr key={s.id} onMouseEnter={e => (e.currentTarget.style.background = C.gray50)} onMouseLeave={e => (e.currentTarget.style.background = '')}>
                      ...
                      <td style={tdStyle}>
                        <button
                          onClick={() => router.push(`/hr`)}
                          style={btnSecondary}
                          title="인사 마스터에서 직원 행 클릭 → 「💼 급여 설정」 탭"
                        >→ 조직/권한에서 편집</button>
                      </td>
                    </tr>
                  )
                ...
                {settings.length === 0 && (
                  <tr>
                    <td colSpan={8} style={{ ...tdStyle, textAlign: 'center', padding: '32px 16px', color: C.gray400, fontSize: 13 }}>
                      등록된 급여 설정이 없습니다 — 「인사 마스터」 페이지에서 직원 선택 후 「💼 급여 설정」 탭에서 입력하세요.
                    </td>
                  </tr>
                )}
              </tbody>
