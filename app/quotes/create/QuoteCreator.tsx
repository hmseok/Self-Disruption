    ...
    return (
      <div className="max-w-[800px] mx-auto py-8 px-4 min-h-screen bg-gray-50/50">
        <div className="flex items-center gap-2 mb-6">
          <Link href="/quotes/create" className="text-slate-500 hover:text-slate-400 text-sm">&larr; 산출 빌더</Link>
          <span className="text-slate-400">/</span>
          <span className="text-steel-600 font-bold text-sm">견적서 작성</span>
        </div>

        <h1 className="text-2xl font-black text-slate-800 mb-2">견적서 작성</h1>
        <p className="text-slate-500 text-sm mb-8">렌트가 산출 결과를 바탕으로 고객용 견적서를 생성합니다.</p>

        {/* 분석 요약 */}
        <div className="bg-gray-50 text-white rounded-2xl p-5 mb-6">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-slate-500 text-xs">분석 차량</p>
              <p className="font-black text-lg">{car.brand} {car.model}</p>
              <p className="text-slate-500 text-sm">{car.trim} · {car.year}년식</p>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-black/[0.06] shadow-sm p-6 mb-4">
          ...
          {customerMode === 'select' ? (
            <>
              <select
                className="w-full p-3 border border-black/[0.06] rounded-xl font-bold text-base focus:border-steel-500 outline-none mb-3"
        ...
        <div className="bg-white rounded-2xl border border-black/[0.06] shadow-sm p-6 mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="border border-black/5 bg-gray-50 rounded-lg px-3 py-2 font-bold text-sm text-slate-400">
                {fDate(endDate)}
              </div>
            </div>
            <div className="mt-5 text-sm text-slate-500 font-bold">{data.termMonths}개월</div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <Link href="/quotes/create"
            className="flex-1 py-3 text-center border border-black/10 rounded-xl font-bold text-slate-500 hover:bg-gray-50">
            &larr; 산출 빌더로
          </Link>
          <button
            onClick={() => {
            ...
            className="flex-[2] py-3 bg-gray-50 text-white rounded-xl font-black hover:bg-gray-100 transition-colors"
          >
            견적서 미리보기 &rarr;
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-6 px-4">
      ...
      <div className="max-w-[800px] mx-auto mb-4 flex justify-between items-center print:hidden">
        <button onClick={() => setStep('setup')} className="text-sm text-slate-500 hover:text-slate-600 font-bold">
          &larr; 설정으로 돌아가기
        </button>
        <div className="flex gap-2">
          <button onClick={handlePrint}
            className="px-4 py-2 border border-black/10 rounded-lg text-sm font-bold text-slate-400 hover:bg-white">
            인쇄
          </button>
          <button onClick={() => handleSaveQuote('draft')} disabled={saving}
            className="px-4 py-2 border border-black/10 rounded-lg text-sm font-bold text-slate-400 hover:bg-white disabled:opacity-50">
            {saving ? '저장중...' : '임시저장'}
          </button>
          <button onClick={() => handleSaveQuote('active')} disabled={saving}
            className="px-5 py-2 bg-gray-50 text-white rounded-lg text-sm font-black hover:bg-gray-100 disabled:opacity-50">
            {saving ? '저장중...' : '견적서 확정'}
          </button>
        </div>
      </div>
      ...
      <div ref={printRef} className="max-w-[800px] mx-auto bg-white rounded-2xl shadow-xl overflow-hidden print:shadow-none print:rounded-none">
        ...
        {/* 헤더 */}
        <div className="bg-gray-50 text-white px-8 py-6">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-black tracking-tight">장기렌트 견적서</h1>
              <p className="text-slate-500 text-sm mt-1">LONG-TERM RENTAL QUOTATION</p>
            </div>
            <div className="text-right">
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">임대인</p>
              <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1">
            ...
            <div>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">임차인</p>
              <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1">
                <p className="font-black text-base">{selectedCustomer?.name || '-'}</p>
                {selectedCustomer?.business_number && <p className="text-slate-500">사업자번호: {selectedCustomer.business_number}</p>}
                {selectedCustomer?.phone && <p className="text-slate-500">연락처: {selectedCustomer.phone}</p>}
                {selectedCustomer?.email && <p className="text-slate-500">{selectedCustomer.email}</p>}
                {selectedCustomer?.address && <p className="text-slate-500">{selectedCustomer.address}</p>}
              </div>
            </div>
          </div>
          ...
          {/* 2. 차량 정보 */}
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">차량 정보</p>
            <div className="border border-black/[0.06] rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <tbody>
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500 w-28">차종</td>
                    <td className="px-4 py-2.5 font-black">{car.brand} {car.model}</td>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500 w-28">트림</td>
                    <td className="px-4 py-2.5 font-bold">{car.trim || '-'}</td>
                  </tr>
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">연식</td>
                    <td className="px-4 py-2.5">{car.year}년</td>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">연료</td>
                    <td className="px-4 py-2.5">{car.fuel || '-'}</td>
                  </tr>
                  <tr>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">차량가격</td>
                    <td className="px-4 py-2.5 font-bold">{f(data.factoryPrice)}원</td>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">차량번호</td>
          ...
          {/* 3. 계약 조건 */}
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">계약 조건</p>
            <div className="border border-black/[0.06] rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <tbody>
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500 w-28">계약유형</td>
                    <td className="px-4 py-2.5 font-black">
                      {data.contractType === 'buyout' ? '인수형 장기렌트' : '반납형 장기렌트'}
                    </td>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500 w-28">계약기간</td>
                    <td className="px-4 py-2.5 font-bold">{data.termMonths}개월</td>
                  </tr>
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">시작일</td>
                    <td className="px-4 py-2.5">{fDate(startDate)}</td>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">종료일</td>
                    <td className="px-4 py-2.5">{fDate(endDate)}</td>
                  </tr>
                  <tr>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">약정주행</td>
                    <td className="px-4 py-2.5">연 {f(data.annualMileage * 10000)}km (총 {f(totalMileageLimit)}km)</td>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">정비상품</td>
          ...
          <div className="border-2 border-gray-900 rounded-2xl overflow-hidden">
            <div className="bg-gray-50 text-white px-6 py-3">
            ...
            <div className="p-6">
              ...
              <div className={`grid ${data.contractType === 'buyout' ? 'grid-cols-3' : 'grid-cols-2'} gap-4 mb-5`}>
                ...
                <div className="text-center p-4 bg-gray-50 rounded-xl">
                  <p className="text-xs text-slate-500 font-bold mb-1">보증금 (계약 시 1회)</p>
                  <p className="text-xl font-black text-slate-700">{f(data.deposit)}<span className="text-sm font-bold">원</span></p>
              ...
              {data.prepayment > 0 && (
                <div className="bg-gray-50 rounded-lg px-4 py-2 mb-4 flex justify-between items-center text-sm">
                  <span className="text-slate-500">선납금 (계약 시 1회)</span>
                  ...
                  <tbody>
                    {data.deposit > 0 && (
                      <tr className="border-b border-black/[0.06]">
                        <td className="py-2 text-slate-500">보증금</td>
                        <td className="py-2 text-right font-bold">{f(data.deposit)}원</td>
                      </tr>
                    )}
                    {data.prepayment > 0 && (
                      <tr className="border-b border-black/[0.06]">
                        <td className="py-2 text-slate-500">선납금</td>
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">렌탈료 포함 서비스</p>
            <div className="border border-black/[0.06] rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-black/[0.06]">
                    <th className="px-4 py-2.5 text-left font-bold text-slate-500 w-40">항목</th>
                    <th className="px-4 py-2.5 text-center font-bold text-slate-500 w-20">포함</th>
                    <th className="px-4 py-2.5 text-left font-bold text-slate-500">상세</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-black/5">
                    ...
                    <td className="px-4 py-2.5 text-slate-400">
                      ...
                      {data.deductible > 0 && (
                        <span className="text-slate-500"> · 자차 면책금 {f(data.deductible)}원</span>
                      )}
                    </td>
                  </tr>
                  <tr className="border-b border-black/5">
                    ...
                    <td className="px-4 py-2.5 text-center text-green-600 font-bold">O</td>
                    <td className="px-4 py-2.5 text-slate-400">계약기간 내 자동차세 전액 포함</td>
                  </tr>
                  <tr className="border-b border-black/5">
                  ...
                  <tr className="border-b border-black/5">
                    ...
                    <td className="px-4 py-2.5 text-center text-green-600 font-bold">O</td>
                    <td className="px-4 py-2.5 text-slate-400">영업용 취득세 4% 포함 (자동차대여업 기준)</td>
                  </tr>
                  <tr className="border-b border-black/5">
                  ...
                  <tr>
                    ...
                    <td className="px-4 py-2.5 text-center text-green-600 font-bold">O</td>
                    <td className="px-4 py-2.5 text-slate-400">번호판(영업용 허/하/호) · 인지세 · 등록대행비 포함</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">약정 조건</p>
            <div className="border border-black/[0.06] rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <tbody>
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500 w-36">약정 주행거리</td>
                    <td className="px-4 py-2.5">
                      연간 {f(data.annualMileage * 10000)}km (계약기간 총 {f(totalMileageLimit)}km)
                    </td>
                  </tr>
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">초과주행 요금</td>
                    <td className="px-4 py-2.5">
                      <span className="font-bold text-red-500">km당 {f(excessMileageRate)}원</span>
                      <span className="text-slate-500 text-xs ml-2">(약정거리 초과 시 계약 종료 시점 정산)</span>
                    </td>
                  </tr>
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">자차 면책금</td>
                    <td className="px-4 py-2.5">
                  ...
                  <tr className="border-b border-black/5">
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">중도해지</td>
                    <td className="px-4 py-2.5 text-slate-400">
                      잔여 렌탈료의 30~40% 위약금 발생 (잔여 기간에 따라 차등 적용)
                    </td>
                  </tr>
                  <tr>
                    <td className="bg-gray-50 px-4 py-2.5 font-bold text-slate-500">반납 조건</td>
                    <td className="px-4 py-2.5 text-slate-400">
                      {data.contractType === 'buyout'
          ...
          {data.contractType === 'buyout' && (
            <div>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">인수 안내</p>
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
                <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-600">취득세 + 이전등록비 별도</p>
                  </div>
                </div>
          ...
          {quoteNote && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
              ...
              <p className="text-sm text-slate-600 whitespace-pre-wrap">{quoteNote}</p>
            </div>
          )}

          {/* 9. 유의사항 */}
          <div className="border-t border-black/[0.06] pt-5">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">유의사항 및 특약</p>
          ...
          <div className="grid grid-cols-2 gap-8 pt-6">
            ...
            <div className="text-center">
              ...
              <div className="border-t border-black/10 pt-2">
                <p className="text-sm font-bold text-slate-600">{selectedCustomer?.name || '고객명'}</p>
              </div>
            </div>
          </div>
        ...
        {/* 푸터 */}
        <div className="bg-gray-50 px-8 py-4 border-t border-black/[0.06] text-center">
      ...
      <div className="max-w-[800px] mx-auto mt-4 flex gap-3 print:hidden">
        <button onClick={() => setStep('setup')}
          className="flex-1 py-3 border border-black/10 rounded-xl font-bold text-slate-500 hover:bg-white">
          &larr; 수정
        </button>
        <button onClick={handlePrint}
          className="flex-1 py-3 border border-black/10 rounded-xl font-bold text-slate-400 hover:bg-white">
          인쇄 / PDF
        </button>
        <button onClick={() => handleSaveQuote('draft')} disabled={saving}
          className="flex-1 py-3 bg-gray-600 text-white rounded-xl font-bold hover:bg-gray-50 disabled:opacity-50">
          임시저장
        </button>
        <button onClick={() => handleSaveQuote('active')} disabled={saving}
          className="flex-[2] py-3 bg-gray-50 text-white rounded-xl font-black hover:bg-gray-100 disabled:opacity-50">
          {saving ? '저장 중...' : '견적서 확정'}
        </button>
      </div>
    </div>
