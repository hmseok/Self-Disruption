...
export default function QuoteCalculatorPage() {
  ...
  const [customers, setCustomers] = useState<any[]>([]) // 🔥 고객 목록 추가

  // 1. 선택된 데이터
  const [selectedCar, setSelectedCar] = useState<any>(null)
  const [selectedCustomerName, setSelectedCustomerName] = useState('') // 🔥 선택된 고객명
  ...
  const [costs, setCosts] = useState({
    monthly_finance: 0, monthly_insurance: 0, maintenance: 50000, total_cost: 0
  })
  ...
  const handleSaveQuote = async () => {
    ...
    if (!selectedCustomerName) return alert('고객을 선택해주세요.') // 🔥 유효성 검사 추가

    const { error } = await supabase.from('quotes').insert([{
        car_id: selectedCar.id,
        customer_name: selectedCustomerName, // 🔥 실제 선택한 고객 이름 저장
        rental_type: '월렌트',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl border shadow-sm">
            ...
            {selectedCar && (
                <div className="mt-4 p-4 bg-gray-50 rounded-xl text-sm text-gray-600 flex justify-between">
                    <span>취득가액: <b>{f(selectedCar.purchase_price)}원</b></span>
                    <span>연료: <b>{selectedCar.fuel}</b></span>
                </div>
            )}
          </div>
