...
export default function LoginPage() {
  ...
  // 화면 상태: 'login' | 'signup-select' | 'signup-email'
  const [view, setView] = useState<'login' | 'signup-select' | 'signup-email'>('login')

  // 🔒 [추가] 비밀번호 유효성 검사 함수 (8자리 + 영문/숫자/특수문자)
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (view === 'signup-email') {
        ...
        if (!validatePassword(password)) {
            return setMessage({
                text: '비밀번호는 영문, 숫자, 특수문자를 포함해 8자리 이상이어야 합니다.',
                type: 'error'
            })
        }

        // 2. 비밀번호 일치 검사
        if (password !== passwordConfirm) return setMessage({ text: '비밀번호가 서로 일치하지 않습니다.', type: 'error' })
    }
    ...
    try {
      if (view === 'signup-email') {
        ...
        if (data.session) {
          ...
          setMessage({ text: '✅ 가입 인증 메일을 보냈습니다! 메일함을 확인해주세요.', type: 'success' })
        }
      } else {
        // 🔵 [로그인]
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          {view === 'login' && (
            <>
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                ...
                {message && <div className={`text-sm font-bold ${message.type==='error'?'text-red-500':'text-green-600'}`}>{message.text}</div>}
            ...
            <div className="space-y-4">
               <button onClick={handleGoogleLogin} className="w-full flex items-center justify-between p-4 border border-gray-200 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50 hover:shadow-md transition-all">
                 <div className="flex items-center gap-3">
                    <div className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full text-lg">G</div>
                    <div className="text-left"><p className="font-bold text-gray-800 text-sm">Google로 시작하기</p></div>
                 </div>
               ...
               <button onClick={() => setView('signup-email')} className="w-full flex items-center justify-between p-4 border border-gray-200 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50 hover:shadow-md transition-all">
                 <div className="flex items-center gap-3">
                    <div className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full text-lg">✉️</div>
                    <div className="text-left"><p className="font-bold text-gray-800 text-sm">이메일로 시작하기</p></div>
                 </div>
                 <span className="text-indigo-500">→</span>
               </button>
            </div>
          ...
          {view === 'signup-email' && (
            <form onSubmit={handleAuth} className="space-y-4 animate-fade-in-up">
                <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1">이름 (실명)</label>
                ...
                <div>
                    ...
                    <input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:bg-white focus:border-indigo-500 font-bold" placeholder="8자리 이상 (영문/숫자/특수문자)" />
                </div>
                <div>
