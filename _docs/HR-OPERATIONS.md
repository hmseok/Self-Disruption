...
## 9. 라이드 직원 계정 / 독립 브랜딩 정책 (2026-05-16 사용자 결정)

### 9.1 라이드 직원 ERP 계정 — 옵션 (a) 채택
- **기본**: 라이드 직원 = `ride_employees` (인사 마스터). 본 ERP 계정 X.
- **예외 (매니저급)**: 추가로 「외부 매니저」 ERP 계정 (`profiles`) 부여.
  - 부서: FMI `departments` 의 「라이드주식회사」
  - 페이지 권한: **라이드 관련 하위 메뉴 위주** — HR 담당자가 직접 설정 (보안 유지하며 파트별 개방)
  - 「라이드 페이지들은 각 파트에 열어줘도 됨」 (사용자 명시)
  - 급여: 본 시스템 X (라이드 자체)
- **다른 외주 직원 (라이드 외)**: 정책 미정 — 향후 결정 (사용자 고민 중)

### 9.2 라이드 독립 브랜딩 (요구 — 메인 세션 작업)
> 사용자 명시: 「메일·메인 로그인 페이지도 라이드주식회사 나와야 한다. FMI 나오면 안 됨. 독립적으로」
- 초대 메일: 라이드 직원 → 발신 회사명 「라이드주식회사」 (「주식회사 에프엠아이」 아님)
- 로그인 페이지: 라이드 계정 로그인 시 「라이드주식회사」 브랜딩
- 회사 판별 기준: **인터뷰 필요** — 이메일 도메인(@rideoffice.kr) / 부서 / 별도 company 필드 중 결정
- 영역: `InviteModal` / `app/page.tsx`(로그인) / 이메일 템플릿 — 메인 세션 책임 (Rule 21)

### 9.3 인사 마스터 동기화 (요구 — hr ↔ 메인 협업)
- `profiles` (ERP 계정) ↔ `ride_employees` (인사 마스터) 의 이름·연락처 자동 동기화
- 「내 정보」 페이지 — 전체 직원 오픈 (requirePermission 조정 — 메인 세션)
- 동기화 방향/연결키 (profile_id FK 활용) — 설계 필요

### 9.4 InviteModal 페이지 권한 동기화 버그 (메인 세션 위탁)
- `InviteModal.tsx` 의 `PAGE_GROUPS` 가 hardcoded → `lib/menu-registry.ts` 와 불일치
