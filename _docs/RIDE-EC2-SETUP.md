...
| 인스턴스 명명 | `ride-{module}-{role}-{env}` | `ride-care-op-server-{dev\|prd}` |
...
| 레포 명명 | `ride-{name}-{tech}` (예: `ride-charger-next`) | `RIDE-CARE-OP-NEXT` 권장 |
...
| 항목 | 권장 | 결정 |
|---|---|---|
| 레포 이름 (Ride-IT org) | `RIDE-CARE-OP-NEXT` | ✅ 확정 (2026-05-31) |
| EC2 이름 (dev) | `ride-care-op-server-dev` | ✅ 확정 |
| EC2 이름 (prd) | `ride-care-op-server-prd` | ✅ 확정 |
| 도메인 | 라이드 회사 보유 도메인 → 서브도메인 (예: `app.ride-care.kr` / `app-dev.ride-care.kr`) | ☐ |
| 보안 그룹 | 신규 `ride-care-op-sg` (포트 22 본인IP / 80·443·3001 전체) | ☐ |
| DB 위치 | 시작 EC2 자체 MySQL → 안정화 후 RDS 분리 | ☐ |
| 코드 분리 | 1단계: 같은 코드 + `MODULE_PROFILE=ride` / 2단계: 새 레포 fork | ☐ |

---

## 1. GitHub — `Ride-IT/RIDE-CARE-OP-NEXT` 레포 신설
...
```bash
# 본 PC 또는 임시 빌드 서버에서
git clone git@github.com:hmseok/Self-Disruption.git RIDE-CARE-OP-NEXT
cd RIDE-CARE-OP-NEXT

# 본 origin 제거 + 라이드 회사 origin 등록
git remote remove origin
git remote add origin git@github.com:Ride-IT/RIDE-CARE-OP-NEXT.git
...
2. 할당된 EIP 우클릭 → **태그**: `Name = eip-ride-care-op-server-dev`
...
```bash
chmod 400 ~/Downloads/ride-care-op-keypair.pem
ssh -i ~/Downloads/ride-care-op-keypair.pem ubuntu@<EIP>
```
...
CREATE DATABASE ride_op CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- 앱 전용 사용자
CREATE USER 'ride_app'@'localhost' IDENTIFIED BY '여기_강한_비밀번호_16자_이상';
GRANT ALL PRIVILEGES ON ride_op.* TO 'ride_app'@'localhost';
FLUSH PRIVILEGES;
...
```bash
mysql -u ride_app -p ride_op -e "SHOW TABLES;"
```
...
```bash
sudo mkdir -p /opt/ride-care-op
sudo chown ubuntu:ubuntu /opt/ride-care-op
cd /opt/ride-care-op

# SSH Deploy Key 등록 후 (Step 1.4)
git clone git@github.com:Ride-IT/RIDE-CARE-OP-NEXT.git .
```
...
ls migrations/ | grep -E "ride|cs_|compliance|asset" | while read f; do
  echo "applying: $f"
  mysql -u ride_app -p ride_op < "migrations/$f"
done
```
...
module.exports = {
  apps: [{
    ...
    script: 'npm',
    args: 'start',
    cwd: '/opt/ride-care-op',
    env: { NODE_ENV: 'production', PORT: 3001 },
    instances: 1,
    autorestart: true,
    max_memory_restart: '768M',   // t2.small 안전 마진
    error_file: '/var/log/ride-care-op/error.log',
    out_file: '/var/log/ride-care-op/out.log',
    time: true,
  }],
}
```
...
```bash
sudo mkdir -p /var/log/ride-care-op
sudo chown ubuntu:ubuntu /var/log/ride-care-op
...
```bash
sudo nano /etc/nginx/sites-available/ride-care-op
```
...
```bash
sudo ln -s /etc/nginx/sites-available/ride-care-op /etc/nginx/sites-enabled/
...
```bash
ssh-keygen -t ed25519 -C "ride-care-op-dev"   # passphrase 빈칸
cat ~/.ssh/id_ed25519.pub                      # 출력 복사
```

Ride-IT 의 `RIDE-CARE-OP-NEXT` 레포 → Settings → Deploy keys → Add deploy key (write 권한 X — pull only).
...
```bash
pm2 status
pm2 logs ride-care-op --lines 50
df -h && free -h
...
mysqldump -u root -p"비밀번호" ride_op | gzip > /opt/backup/db/ride_op-$TS.sql.gz
find /opt/backup/db -name "*.sql.gz" -mtime +30 -delete

# S3 외부 백업 (강력 권장)
aws s3 cp /opt/backup/db/ride_op-$TS.sql.gz s3://ride-care-op-backups/db/
```
...
| 종류 | 이름 |
|---|---|
| GitHub 레포 | `Ride-IT/RIDE-CARE-OP-NEXT` |
| EC2 dev | `ride-care-op-server-dev` |
| EC2 prd | `ride-care-op-server-prd` |
| EIP dev | `eip-ride-care-op-server-dev` |
| EIP prd | `eip-ride-care-op-server-prd` |
| 보안 그룹 | `ride-care-op-sg` |
| 키페어 | `ride-care-op-keypair` |
| IAM 역할 | `ride-care-ec2-roles` (기존 재사용) |
| DB | `ride_op` |
| DB 사용자 | `ride_app` |
| 도메인 | `app.ride-care.kr` (prd) / `app-dev.ride-care.kr` (dev) |
| 코드 디렉토리 | `/opt/ride-care-op` |
| 로그 디렉토리 | `/var/log/ride-care-op` |
| PM2 앱명 | `ride-care-op` |
| S3 버킷 | `ride-care-op-backups` |
