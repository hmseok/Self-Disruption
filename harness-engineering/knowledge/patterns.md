...
### SMS 웹훅 수신 패턴 (SMS Forwarder 앱)
```typescript
// 1. 전처리 체인 (순서 중요)
//    ① "보낸사람 : 01050349550" 접두어 제거 + 발신번호 추출
//    ② "[Web발신]" 태그 제거
//    ③ 파서 실행

// 2. 파서는 반드시 2가지 포맷 모두 지원
//    포맷 A (기존): [KB국민] 홍길동 4/21 14:32 CU편의점 3,500원 일시불 승인
//    포맷 B (실제): KB국민카드 8819(기업) 홍길동 04/21 14:32 CU편의점 3,500원 일시불 승인

// 3. 중복 방지: raw_hash = sha256(from|text) → DB unique check
// 4. 파싱 실패 시 raw_text 저장 → 재파싱 API로 복구 가능
```

### 외부 연동 개발 원칙 (실제 데이터 우선)
```
1. 연동 대상 앱/서비스 설치 + 설정
2. 테스트 데이터 전송 → 실제 raw 페이로드 캡처
3. 캡처한 데이터 기반으로 파서/핸들러 구현
4. 절대 "이론적 포맷"으로 먼저 구현하지 않는다
```
...
단순 (버그 수정, UI 미세 조정):
  Generator → 빌드 확인 → 커밋
  예: 색상 변경, 텍스트 수정, 단순 필터 추가

중간 (기능 추가, API 신규):
  Researcher(간단 조사) → Generator → Reviewer → 커밋
  예: 새 탭 추가, CRUD API, 필터 기능

대규모 (아키텍처 변경, 신규 모듈):
  풀 파이프라인 (GATE 1~9)
  예: 계산엔진 재설계, 새 도메인 모듈, DB 스키마 대규모 변경
```
