...
0. **매핑 관리 다이어트** (7-05 사용자 명시 「카드별 통장별 수집구분만 하면 되고 복잡하게 할 필요 없다」):
   bank_account_mappings / 카드 매핑 UI 를 「통장 식별 / 카드 식별」 두 목록으로 단순화.
   매칭은 이제 사고차량번호·입금자명·고객명·expected_payer 축 자동 — 계좌별 차량배정 등 복잡 기능 불필요.
0-1. **배포 후 복구 절차** (매칭 모수): 관리자 SMS 거래 재생성 1회 → 자동매칭 1회 → 청구 탭 💰 확인.
