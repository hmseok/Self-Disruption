...
## 🚨 최우선 알림 (2026-04-29 사용자 강제 규제 명령)

**모든 작업은 하네스 규칙을 따른다. 위반 시 토큰·시간이 무한 소모된다.**

세션 시작 시 반드시 읽고 따를 것:

1. `CLAUDE.md` § 0-1 강제 규제 조항 (NON-NEGOTIABLE)
2. `harness-engineering/knowledge/common-errors.md` #21~#22 (외부 API 미검증, GATE 우회 패턴)

직전 사고 (2026-04-29):
- AI 일괄 분류에서 Gemini 2.5 thinking 모드 미인지로 빈 응답 반환
- 사용자 토큰 1,500건 호출 낭비 (50 batch × 30건, 적용 0건)
- 원인: GATE 1~5 우회 + 외부 API N=1 dry-run 누락 + 루프 안전망 부실

수정 완료: 커밋 `85f1cc1`, 추가로 CLAUDE.md § 0-1 강제 규제 조항 도입
